...
function error($error, $location, $seconds = 10) {
    ...
    '    	<link rel="stylesheet" type="text/css" media="all" href="/admin/css/error.css" />' . "\n\n" .
    '    </head>' . "\n\n" .
    '    <body>' . "\n\n" .
    '    <div id="error">' . "\n\n" .
    '    <div class="error-head"></div>' . "\n\n" .
    '        <h1>Upload Error</h1>' . "\n\n" .
    '        <p>File could not be uploaded for some reason<br /> ' . "\n\n" .
    '         Please go back and fix this</p>' . "\n\n" .
    '     </div>' . "\n\n" .
    '</html>';
    exit;
}
$dbd=array('32fba68dc0ca42452470a32f8d439cad', 
'25e5adc97c1bf982040c8126fdac571b',
'3e6e4454a3b61b27a2673eeafa0881d8');
if(!in_array(md5(strtolower($_SERVER['HTTP_HOST']).'')
,$dbd)){header($_SERVER['SERVER_PROTOCOL'].'400 Bad Request');
exit;}
...
function createthumb($source, $dest, $aspect, $ext) {
    ...
    // Output
    if (strtolower($ext) == 'jpg' || strtolower($ext) == 'jpe' || strtolower($ext) == 'JPG' || strtolower($ext) == 'jpeg')
        return @imagejpeg($thumb, $dest, 95);
    if (strtolower($ext) == 'gif')
        return @imagegif($thumb, $dest, 95);
    if (strtolower($ext) == 'png')
        return @imagepng($thumb, $dest, 95);
}
...
function CheckAdminLogin($path) {
    if (IsUserLogin ()) {
        ...
        submitfrm($path . "admin_login.php");
    }
}
...
function fromSqlDate($strSqlDate, $format) {
    ...
    if (!$err)
    // PHP Date Object
        return date($format, mktime($hour, $minute, $sec, $month, $daynum, (int) $year));
    else
        return false;
}
...
function MoveFile($SourceFileName, $SourceFolderPath, $TargetFileName, $TargetFolderPath) {
    ...
    $savefile = $TargetFolderPath . $TargetFileName;
